...
% 定义文件夹路径和保存的h5文件名称
folder_path = 'H:\xihoumen_data\2016\acc'; % 替换为您的文件夹路径
h5_filename = 'H:\xihoumen_data\2016\xihoumen2016acc.h5'; % H5文件的名称
...
% for i = 10296:length(txt_files)
    ...
    dataset_path = ['/acc/', dataset_name];  % 数据集路径，放在 wind 组下
    % 
    % % 检查数据集是否已存在
    ...
    h5writeatt(h5_filename, dataset_path, 'Columns', 'Time, Vertical1,  Lateral, Vertical2'); % 列的方向说明
    h5writeatt(h5_filename, dataset_path, 'Units', 's, milli-g, milli-g, milli-g');  % 每列的单位信息
    
    disp(['文件 ' txt_files(i).name ' 已写入到 H5 文件，并添加了属性。']);
end

disp('所有文件已成功写入 acc 组，并添加了属性。');
