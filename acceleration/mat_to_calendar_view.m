...
for k1 = 1:num_days
    ...
    for k2 = 1:24
        ...
        fileName{k1,k2}= sprintf("%s %02d.mat", baseName, k2-1);
    end
end
...
plotFns = cell(num_days, 1); % Assuming September (30 days)
...
for k1 = 1:num_days
    if ~isempty(dailyDataTables{k1})
        ...
        % yData1 = dailyDataTables{k1}.AC2_1;
        % yData2 = dailyDataTables{k1}.AC2_2;
        % yData3 = dailyDataTables{k1}.AC2_3;
        yData4 = dailyDataTables{k1}.AC3_1;
        % yData5 = dailyDataTables{k1}.AC3_2;
        % yData6 = dailyDataTables{k1}.AC3_3;
        % yData7 = dailyDataTables{k1}.AC4_1;
        % yData8 = dailyDataTables{k1}.AC4_2;
        % yData9 = dailyDataTables{k1}.AC4_3;
        % % 找到所有数据中的最大绝对值
        % globalMax = max([abs(yData1(:)); abs(yData2(:)); abs(yData3(:)); abs(yData4(:)); abs(yData5(:)); abs(yData6(:)); abs(yData7(:)); abs(yData8(:)); abs(yData9(:))]);
        % 
        % % 使用找到的最大值对所有数据进行缩放
        % yData1 = yData1 / globalMax;
        % yData2 = yData2 / globalMax;
        % yData3 = yData3 / globalMax;
        % yData4 = yData4 / globalMax;
        % yData5 = yData5 / globalMax;
        % yData6 = yData6 / globalMax;
        % yData7 = yData7 / globalMax;
        % yData8 = yData8 / globalMax;
        % yData9 = yData9 / globalMax;

        plotFns{k1} = {
            % @(ax) scatter(ax,timeSeries,yData1), ...
            % @(ax) scatter(ax,timeSeries,yData2), ...
            % @(ax) scatter(ax,timeSeries,yData3), ...
            @(ax) scatter(ax,timeSeries,yData4), ...
            % @(ax) scatter(ax,timeSeries,yData5), ...
            % @(ax) scatter(ax,timeSeries,yData6), ...
            % @(ax) scatter(ax,timeSeries,yData7), ...
            % @(ax) scatter(ax,timeSeries,yData8), ...
            % @(ax) scatter(ax,timeSeries,yData9)
        };
    end
end
...
print(gcf, sprintf('Vibdata for %d-%02d', year, month),'-r300','-dpng');
