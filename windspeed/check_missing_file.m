...
for k1 = 1: numIterations
    baseName = fileList(k1).name(1:end-8); % 仅列出其中一种txt文件来获取文件列表
    % check if file exists
    if ~exist([outputPath baseName '-UA1.txt'], 'file')
        warning('File %s does not exist', [outputPath baseName '-UA1.txt']);
    end
    if ~exist([outputPath baseName '-UA2.txt'], 'file')
        warning('File %s does not exist', [outputPath baseName '-UA2.txt']);
    end
    if ~exist([outputPath baseName '-UA3.txt'], 'file')
        warning('File %s does not exist', [outputPath baseName '-UA3.txt']);
    end
    if ~exist([outputPath baseName '-UA4.txt'], 'file')
        warning('File %s does not exist', [outputPath baseName '-UA4.txt']);
    end
    if ~exist([outputPath baseName '-UA5.txt'], 'file')
        warning('File %s does not exist', [outputPath baseName '-UA5.txt']);
    end
    if ~exist([outputPath baseName '-UA6.txt'], 'file')
        warning('File %s does not exist', [outputPath baseName '-UA6.txt']);
    end
end